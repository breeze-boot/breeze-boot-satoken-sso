...
import com.breeze.boot.security.utils.WxHttpInterfaceUtils;
...
public class WxPhoneAuthenticationProvider implements AuthenticationProvider {
     ...
     * 创建失败
     */
    private static final String CREATE_FAIL = "微信手机号获取失败";
    ...
    public WxPhoneAuthenticationProvider(LocalUserDetailsService userDetailsService,
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        WxPhoneAuthenticationToken authenticationToken = (WxPhoneAuthenticationToken) authentication;
        String accessToken = WxHttpInterfaceUtils.getAccessToken(this.wxLoginProperties.getAppId(), this.wxLoginProperties.getAppSecret());
        String code = String.valueOf(authenticationToken.getPrincipal());
        String phoneNumber = WxHttpInterfaceUtils.getPhoneNumber(code, accessToken);

        UserDetails userDetails = this.userDetailsService.createOrLoadUserByPhone(String.valueOf(phoneNumber));
    ...
    public boolean supports(Class<?> authentication) {
        return WxPhoneAuthenticationToken.class.isAssignableFrom(authentication);
    }
