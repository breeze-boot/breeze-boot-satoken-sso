...
public interface IUserDetailService {
    ...
    default UserPrincipal convertResponseUserInfo(UserInfoDTO userInfo) {
        return new UserPrincipal(userInfo.getUsername(), userInfo.getPassword(), Objects.equals(userInfo.getIsLock(), 0), true, true, Objects.equals(userInfo.getIsLock(), 0), userInfo.getAuthorities().stream().map(GrantedAuthority::new).collect(Collectors.toSet()), userInfo.getUserId(), userInfo.getDeptId(), userInfo.getDeptName(), userInfo.getUserCode(), userInfo.getDisplayName(), userInfo.getAvatar(), userInfo.getPhone(), userInfo.getSex(), userInfo.getAmountType(), userInfo.getIsLock(), userInfo.getEmail(), userInfo.getUserRoleCodes(), userInfo.getUserRoleIds(), userInfo.getTenantId(), userInfo.getPermissionType(), userInfo.getRowPermissionCode(), userInfo.getSubDeptId());
    }
