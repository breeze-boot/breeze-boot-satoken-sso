...
package com.breeze.boot.database.method;
...
import static com.breeze.boot.core.constants.BreezeStrPool.*;
...
public class BreezeInsertAllBatch extends AbstractMethod {
    ...
    private StringBuilder prepareValueSql(TableInfo tableInfo) {
        ...
        valueSql.append(_C_DELIM_START).append(tableInfo.getKeyProperty()).append(C_DELIM_END + "" + C_COMMA);
        tableInfo.getFieldList().forEach(x -> valueSql.append(_C_DELIM_START).append(x.getProperty()).append(C_DELIM_END + "" + C_COMMA));
