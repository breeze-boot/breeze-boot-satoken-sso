...
import com.breeze.boot.auth.mapper.SysRowPermissionMapper;
import com.breeze.boot.auth.model.converter.SysRowPermissionConverter;
...
public class SysRowPermissionServiceImpl extends ServiceImpl<SysRowPermissionMapper, SysRowPermission> implements SysRowPermissionService {
    ...
    public RowPermissionVO getInfoById(Long permissionId) {
        ...
        // 批量添加到缓存中
        String permissionCode = customizePermission.getPermissionCode();
        this.redisTemplate.opsForValue().set(CacheConstants.ROW_PERMISSION + permissionCode, customizePermission);
    ...
    public Result<Boolean> saveRowPermission(RowPermissionForm form) {
        SysRowPermission sysRowPermission = this.sysRowPermissionConverter.form2Entity(form);
        AssertUtil.isFalse(DataPermissionType.checkInEnum(form.getPermissionCode()), NO_ACTION_IS_ALLOWED);
        boolean save = this.save(sysRowPermission);
        AssertUtil.isTrue(save, FAIL);
        CustomizePermission customizePermission = this.sysRowPermissionConverter.entity2Cache(sysRowPermission);
        String permissionsString = sysRowPermission.getPermissions().stream()
                .map(Object::toString)
                .collect(Collectors.joining(","));
        customizePermission.setPermissions(permissionsString);
    ...
    public Result<List<Map<String, Object>>> selectPermissionType() {
        return Result.ok(Arrays.stream(DataPermissionType.values()).map(permission -> {
            ...
            if (StrUtil.equals(CUSTOMIZES.getType(), permission.getType())) {
