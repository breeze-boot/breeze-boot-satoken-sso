...
import com.breeze.boot.auth.mapper.SysUserMapper;
import com.breeze.boot.auth.model.bo.SysDeptBO;
import com.breeze.boot.auth.model.bo.UserBO;
import com.breeze.boot.auth.model.bo.UserRoleBO;
import com.breeze.boot.auth.model.converter.SysUserConverter;
import com.breeze.boot.auth.model.entity.*;
import com.breeze.boot.auth.model.excel.UserExcel;
...
import com.breeze.boot.auth.service.*;
...
import static com.breeze.boot.core.enums.DataPermissionType.CUSTOMIZES;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
    ...
    public Result<Boolean> saveUser(UserForm form) {
        ...
        SysUser sysUser = this.sysUserConverter.form2Entity(form);
    ...
    private boolean saveUserRole(UserForm userForm, Long id) {
        ...
        if (saveBatch) {
            ...
            List<String> userRoleCodeList = (List<String>) SaManager.getSaTokenDao().getObject(ROLE_PERMISSION + id);
            if (userRoleCodeList != null) {
                List<UserRoleBO> userRoleBOList = Optional.ofNullable(this.sysRoleService.listRoleByUserId(id))
    ...
    public Boolean reset(UserResetForm form) {
        ...
        String aesSecret = aesSecretProperties.getAesSecret();
        form.setPassword(BCrypt.hashpw(AesUtil.decryptStr(form.getPassword(), aesSecret), BCrypt.gensalt()));
        return this.update(Wrappers.<SysUser>lambdaUpdate()
                .set(SysUser::getPassword, form.getPassword())
    ...
    public Result<Boolean> setRole(UserRolesForm form) {
        SysUser sysUser = this.getOne(Wrappers.<SysUser>lambdaQuery()
                .eq(SysUser::getId, form.getUserId()));
        AssertUtil.isNotNull(sysUser, USER_NOT_FOUND);
        this.sysUserRoleService.remove(Wrappers.<SysUserRole>lambdaQuery()
                .eq(SysUserRole::getUserId, sysUser.getId()));
    ...
    public SysUser registerUser(SysUser registerUser, String roleCode) {
        ...
        SysUser sysUser = SysUser.builder()
                ...
                .displayName(registerUser.getUsername())
                .password(("123456"))
        ...
        SysRole sysRole = this.sysRoleService.getOne(Wrappers.<SysRole>lambdaQuery()
                .eq(SysRole::getRoleCode, roleCode));
        AssertUtil.isNotNull(sysRole, ROLE_NOT_FOUND);
        this.sysUserRoleService.save(
                SysUserRole
                .builder()
    ...
    public List<SysUser> listDeptsUser(Long deptId) {
        ...
        AssertUtil.isTrue(CollUtil.isNotEmpty(deptIdList), ResultCode.FAIL);
    ...
    public UserInfoDTO buildLoginUserInfo(SysUser sysUser) {
        UserInfoDTO userInfo = this.sysUserConverter.entity2BaseLoginUser(sysUser);
    ...
    private void setDeptName(SysUser sysUser, UserInfoDTO userInfo) {
        Optional.ofNullable(this.sysDeptService.getById(sysUser.getDeptId()))
                .ifPresent(sysDept -> userInfo.setDeptName(sysDept.getDeptName()));
    }

    private Set<Long> getHasNormalPermissionRoleId(List<UserRoleBO> userRoleBOList) {
        // @formatter:off
        return userRoleBOList.stream()
                .filter(permission -> StrUtil.equals(permission.getRowPermissionType(), CUSTOMIZES.getType()))
    ...
    private void setRoleCode(List<UserRoleBO> userRoleBOList, UserInfoDTO userInfo) {
        userInfo.setUserRoleCodes(userRoleBOList.stream()
                .map(UserRoleBO::getRoleCode)
                .collect(Collectors.toSet()));
    }

    private void setUsersRoleId(List<UserRoleBO> userRoleBOList, UserInfoDTO userInfo) {
        Set<Long> roleIds = userRoleBOList.stream()
                .map(UserRoleBO::getRoleId)
                .filter(Objects::nonNull)
                .collect(Collectors.toSet());
    ...
    private String getMaxPermissionScope(List<UserRoleBO> userRoleBOList) {
        ...
        return resultSet.stream().min(Comparator.comparingInt(DataPermissionType::getLevel))
                .map(DataPermissionType::toString).orElse("");
    }
    ...
    public List<String> loadUserRoleByUserId(String userId) {
        List<UserRoleBO> userRoleBOList = this.sysRoleService.listRoleByUserId(Long.valueOf(userId));
        AssertUtil.isTrue(CollUtil.isNotEmpty(userRoleBOList), USERS_ROLE_IS_NULL);
